...
export const DeviceManagement: React.FC = () => {
  const { devices, regions, stores, deviceTypes, updateDevice, addDevice, auditRecords, submitOpsStatusChange, submitInspectionReport } = useApp();
  ...
  // ... (Existing state initialization for Add Device Form) ...
  ...
  const isTotalLimitReached = useMemo(() => Object.keys(CATEGORY_LIMITS).every(cat => (imageCounts[cat] || 0) >= CATEGORY_LIMITS[cat]), [imageCounts]);
