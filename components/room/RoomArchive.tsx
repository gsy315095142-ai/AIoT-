...
import { Store as StoreIcon, Plus, ChevronDown, Search, Edit2, Trash2, ChevronRight, Ruler, RotateCcw, Send, Hammer, Calendar, User, X, FileText, ArrowLeft } from 'lucide-react';
import { useApp } from '../../context/AppContext';
import { Store, Region, RoomTypeConfig } from '../../types';
...
export const RoomArchive: React.FC = () => {
    ...
    // View 2: Store Detail (Room Management) - Basic Implementation to allow viewing/adding rooms
    if (viewingStoreId) {
        ...
        return (
            <div className="h-full flex flex-col bg-white">
                <div className="bg-white p-4 border-b border-slate-100 flex items-center gap-2 shadow-sm">
                    <button onClick={() => setViewingStoreId(null)} className="p-1 -ml-1 hover:bg-slate-100 rounded-full text-slate-500">
                        <ArrowLeft size={20} />
                    </button>
                ...
                <div className="p-4 overflow-y-auto flex-1">
                    {/* Room Type Configs */}
                    <div className="mb-6">
                        <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">房型配置</h4>
                        <div className="space-y-2">
                            {store.roomTypeConfigs.length === 0 && <div className="text-xs text-slate-400 italic">暂无房型</div>}
                            {store.roomTypeConfigs.map(rt => (
                                <div key={rt.id} className="bg-slate-50 p-3 rounded-lg border border-slate-100 flex justify-between items-center">
                    ...
                    {/* Rooms List */}
                    <div>
                        <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">客房列表</h4>
                        <div className="grid grid-cols-3 gap-3">
                            {store.rooms.map((room, idx) => (
                                <div key={idx} className="bg-white border border-slate-200 p-3 rounded-lg text-center">
                                    <div className="font-bold text-slate-800">{room.number}</div>
